...
class ControllerModulepopup extends Controller {
		...
		$this->data['button_cancel'] = $this->language->get('button_cancel');
 		...
 		if (isset($this->error['code'])) {
  		...
  		$this->document->breadcrumbs = array();
   		...
   		$this->document->breadcrumbs[] = array(
       		'href'      => $this->url->https('module/popup'),
       		'text'      => $this->language->get('heading_title'),
      		'separator' => ' :: '
   		);
		
		$this->data['action'] = $this->url->https('module/popup');
		
		$this->data['cancel'] = $this->url->https('extension/module');

		if (isset($this->request->post['popup_code'])) {
			$this->data['popup_code'] = $this->request->post['popup_code'];
		} else {
			$this->data['popup_code'] = $this->config->get('popup_code');
		}	
		
		if (isset($this->request->post['popup_status'])) {
			$this->data['popup_status'] = $this->request->post['popup_status'];
		} else {
			$this->data['popup_status'] = $this->config->get('popup_status');
		}
		
		$this->template = 'module/popup.tpl';
		$this->children = array(
			'common/header',	
			'common/footer'	
		);
	...
	private function validate() {
		...
		if (!$this->request->post['popup_code']) {
