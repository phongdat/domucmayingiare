...
class ControllerProductProduct extends Controller {
	...
	public function index() { 
		...
		if ($product_info) {
			...
			if($product_info['price'] == 0) {$this->data['price'] = $this->language->get('price_contact');}
			$this->load->model('total/tax');
			$this->data['text_tax'] = $this->language->get('text_tax');
			$tax = $this->model_total_tax->getTax($product_info['tax_class_id']);
			if($product_info['tax_class_id']) {
				foreach ($tax as $taxs) {
				$this->data['tax'] = sprintf($this->language->get('yes_tax'), $taxs['description']);
				}
			} else {
			...
			$this->data['promotion'] = html_entity_decode($product_info['promotion'], ENT_QUOTES, 'UTF-8');
...
//san pham lien quan
			...
			$resultsCategoryId = $this->model_catalog_product->getProductsRelatedByCategoryId($product_id, $category_id, $start = 0, $limit = $this->config->get('config_related'));
			
			$resultsRelated = $this->model_catalog_product->getProductsRelated($product_id);
			
			if($resultsRelated) {
				$results = $resultsRelated;
			} else {
				$results = $resultsCategoryId;
			}
